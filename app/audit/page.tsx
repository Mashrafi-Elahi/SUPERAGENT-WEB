...
export default function AuditPage() {
  ...
  return <div className="min-h-screen text-text-primary"><Sidebar /><main className="app-main">
    <header className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between"><div><div className="section-kicker">{t('accountability')}</div><h1 className="mt-2 text-3xl font-extrabold">{t('auditTrailTitle')}</h1><p className="mt-2 text-sm text-text-secondary">{t('auditDescription')}</p></div><ScenarioBadge /></header>
    <div className="mb-5 flex items-center justify-between gap-3 rounded-2xl border border-teal/25 bg-teal-soft p-4 text-sm text-text-secondary"><span className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-teal" />{t('syntheticHistoryOnly')}</span><span className="data-source-pill"><Database className="h-3.5 w-3.5" />{source === 'backend-api' ? t('backendApi') : t('mockFallback')}</span></div>
    <section className="card overflow-hidden"><div className="border-b border-bg-border p-5"><div className="flex items-center gap-2 font-bold"><ClipboardList className="h-5 w-5 text-bkash" />{t('importantWorkflowEvents')}</div><p className="mt-1 text-xs text-text-muted">{t('traceableEvents', { count: events.length })}</p></div><div className="divide-y divide-bg-border">{events.map((event) => <div key={event.id} className="grid gap-2 p-5 sm:grid-cols-[100px_120px_120px_1fr]"><span className="font-mono text-xs text-text-muted">{event.timestamp}</span><span className="font-mono text-xs font-bold text-bkash">{event.caseId}</span><span className="text-xs text-text-secondary">{event.provider}</span><span className="text-sm"><strong>{event.actor}</strong><span className="mt-1 block text-xs text-text-secondary">{event.action}</span></span></div>)}</div></section>
  </main></div>;
}
